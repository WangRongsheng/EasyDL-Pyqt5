...
def makenImgName(path):
    ...
    host = 'https://aip.baidubce.com/oauth/2.0/token?grant_type=client_credentials&client_id=你的AK' \
           '&client_secret=你的SK'
    ...
    request_url = "你的模型api" + "?access_token=" + access_token
...
class Window(QMainWindow):
    ...
    def openf(self):
        ...
        for res_content in res:
            cnt=cnt+1
            if res_content['name'] == 'nomask' and res_content['score'] < 0.6:
                errorKey=1#佩戴不正确
            if res_content['name'] == 'mask':
                errorKey=0#佩戴正确
            if errorKey == 1:
                message = QtWidgets.QMessageBox.warning(self, "结果", "EasyDL识别结果为：图片中口罩佩戴不正确！",
                                                        QtWidgets.QMessageBox.Cancel)
                break
            if errorKey == 0:
                message = QtWidgets.QMessageBox.warning(self, "结果", "EasyDL识别结果为：图片口罩佩戴正确！",
                                                        QtWidgets.QMessageBox.Cancel)
                break
